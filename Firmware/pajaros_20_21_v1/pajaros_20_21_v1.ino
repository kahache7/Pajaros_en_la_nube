...
//kike
  ...
  enum constants{
    ...
    INTERVALO_TX = 10 * 60 * 1000 //kike 10 mitutos entre transmisiones en ms
  };
...
// LoRaWAN
  ...
  uint16_t userChannelsMask[6]={ 0x00FF,0x0000,0x0000,0x0000,0x0000,0x0000 }; //kike default channels 0..7
  ...
  uint32_t appTxDutyCycle = INTERVALO_TX; //kike ms entre transmisiones
...
// ToF
  ...
  uint8_t VL6180X_leer(){
    ToF_init(); //kike TODO es necesario el begin cada vez que salgamos de powerdown?
...
static void prepareTxFrame( uint8_t port )
{
  ...
  delay(50); //kike retardo para asentar Vext. Era 500 para bme280, en el ejempplo del VL6180x 50
...
/* //kike del ejemplo de downlink por si se desea bidireccionalidad...
...
void setup() {
  ...
  //copy credenciales de lora_keys.h
    memcpy(nwkSKey, (void*)mynwkSKey, sizeof(mynwkSKey)); //copy keys from .h
    memcpy(appSKey, (void*)myappSKey, sizeof(myappSKey));
    devAddr = mydevAddr; //es un uint32 memcpy(devAddr, (void*)mydevAddr, sizeof(mydevAddr));
  ...
  pinMode(Vext, OUTPUT);
  digitalWrite(Vext, LOW);
  delay(500); //kike era 500 para bme280, en el ejempplo del VL6180x 50. Esta en setup, solo se hace la primera vez.
